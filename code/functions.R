...
hex_bin <- function(x, y, weight = NULL, var4 = NULL, width = NULL, height = NULL, 
                    ...
                    hmin = 0, hmax = 1, xlim = NULL, ylim = NULL, func=function (x) {t <- table(x); t["T"]/sum(t)}, ...) {
  ...
  binned <-cleaned %>% group_by(x,y, var4) %>% tally() %>% group_by(x,y) %>% filter(n == max(n)) %>% select(-n)
  
  var4_sum <- aggregate(cleaned$var4, by=list(cleaned$x, cleaned$y), FUN=func)
