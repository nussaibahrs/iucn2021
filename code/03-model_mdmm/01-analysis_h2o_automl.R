...
# classification data
df.corals <- read.csv(file.path("data", "traits_iucn_mdmm.csv"), stringsAsFactors = FALSE)# %>% 
#   mutate(
...
opt_par_final <- full_join(opt_train, opt_test)%>%
  ...
  write.csv(file.path("output", paste0("Table_S_maximised_threshold_mdmm_", hours, "h.csv")), row.names = FALSE) 
